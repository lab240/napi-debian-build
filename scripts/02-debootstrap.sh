...
DEBOOTSTRAP_CACHE="${BASEDIR}/cache/debootstrap-${DISTRIBUTION}.tar"

if [[ -f "${DEBOOTSTRAP_CACHE}" ]]; then
    log_info "Распаковка debootstrap из кеша..."
    tar -xf "${DEBOOTSTRAP_CACHE}" -C "${ROOTFS}"
    log_info "Debootstrap восстановлен из кеша"
else
    log_info "Debootstrap ${DISTRIBUTION} arm64 (первый запуск, создаём кеш)..."
    debootstrap --arch=arm64 --include="${PKGS}" --cache-dir="${APT_CACHE}" "${DISTRIBUTION}" "${ROOTFS}"
    log_info "Сохранение debootstrap в кеш..."
    tar -cf "${DEBOOTSTRAP_CACHE}" -C "${ROOTFS}" .
    log_info "Кеш сохранён: ${DEBOOTSTRAP_CACHE}"
fi
