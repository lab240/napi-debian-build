...
# SSH - включаем и разрешаем root по паролю
systemctl enable ssh.service
sed -i 's/^#\?PermitRootLogin.*/PermitRootLogin yes/' /etc/ssh/sshd_config
CHROOT_EOF
log_info "Система настроена"

sed -i "s/127.0.0.1\tlocalhost/127.0.0.1\tlocalhost ${HOSTNAME_TARGET}/" "${ROOTFS}/etc/hosts"
log_info "Hostname добавлен в /etc/hosts"

# --- Цветной prompt для root и napi ---
...
cat > "${ROOTFS}/home/napi/.bashrc" <<'BASHRC'
export LS_OPTIONS='--color=auto'
eval "$(dircolors)"
alias ls='ls $LS_OPTIONS'
alias ll='ls -lah'
...
BASHRC
chown 1000:1000 "${ROOTFS}/home/napi/.bashrc"
log_info "Цветной prompt настроен"
# --- MOTD ---
